...
from .utils.tools import AverageMetric, MedianMetric, q90Metric, RecallMetricOld, RecallMetric, set_seed, fork_rng
    ...
    for data in tqdm(loader, desc='Evaluation', ascii=True, disable=not pbar):
        ...
        for k, v in numbers.items():
            if k not in results:
                ...
                if k in conf.q90_metrics:
                    results[k+'_q90'] = q90Metric()
                if 'bin_rec' in k:
                    results[k] = RecallMetricOld()
                if 'L1' in k:
                    for thresh in [1, 2, 5, 10]:
                        results[k+'_recall_'+str(thresh)] = RecallMetric(threshold=thresh, method='non-smooth')
#                         results[k+'_recall_'+str(thresh)+'_smooth'] = RecallMetric(threshold=thresh, method='smooth')
                        
            results[k].update(v)
            if k in conf.median_metrics:
                results[k+'_median'].update(v)
            if k in conf.q90_metrics:
                results[k+'_q90'].update(v)
            if 'L1' in k:
                for thresh in [1, 2, 5, 10]:
                    results[k+'_recall_'+str(thresh)].update(v)
#                     results[k+'_recall_'+str(thresh)+'_smooth'].update(v)
...
def training(rank, conf, output_dir, args):
    ...
    while epoch < conf.train.epochs and not stop:
        ...
        for it, data in enumerate(train_loader):
            ...
            if ((tot_it % conf.train.eval_every_iter == 0) or stop
                    or ((it == (len(train_loader)-1)) and epoch==conf.train.epochs-1)):
